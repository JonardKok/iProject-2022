Create view VWaantalrubrieken as
WITH  nodes AS (
         SELECT R.rubrieknummer as id, COALESCE(rubriekparent, 0) AS idParentCategory
              , count(*) as sumSubtotal, month(looptijdbeginDag) as maand, YEAR(looptijdbeginDag) as jaar
           FROM Rubriek R
		   ...
		   group by YEAR(looptijdbeginDag),datename(month, looptijdbeginDag), R.rubrieknummer, rubriekparent, looptijdbeginDag
          UNION
         SELECT -1, null, -1, 1, '2010'
     )
   , cte AS (
