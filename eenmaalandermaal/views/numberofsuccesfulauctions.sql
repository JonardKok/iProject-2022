create VIEW VWaantalgeslaagdeveilingen as
WITH  nodes AS (
         SELECT R.rubrieknummer as id, COALESCE(rubriekparent, 0) AS idParentCategory
              , sum(case when verkoopprijs is not null then 1 else null end) as sumSubtotal, month( looptijdbeginDag) as maand, YEAR(looptijdbeginDag) as jaar
           FROM Rubriek R
